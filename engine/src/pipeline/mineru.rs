...
use crate::{events, now, state::AppState};
...
pub async fn parse(
    ...
    let batch = if let Some(batch) = existing {
        ...
        sqlx::query(concat!(
            "UPDATE documents SET mineru_task_id=$2,updated_at=",
            now!(),
            " WHERE id=$1"
        ))
            .bind(id)
    ...
    loop {
        ...
        if let Some(result) = result {
            ...
            if raw == "failed" {
                // A failed batch fails again on resume; the next manual retry
                // must upload the source into a new batch.
                sqlx::query("UPDATE documents SET mineru_task_id=NULL WHERE id=$1")
                    .bind(id)
                    .execute(&state.pool)
                    .await?;
                return Err(super::permanent(format!(
                    "MinerU 解析失败：{}",
            ...
            sqlx::query(concat!(
                "UPDATE documents SET stage=$2,progress=$3,pages_processed=$4,pages_total=$5,updated_at=",
                now!(),
                " WHERE id=$1 AND status='processing'"
            ))
            .bind(id)
            .bind(format!("mineru_{raw}"))
            .bind(percent)
            .bind(done)
            .bind(total)
            .execute(&state.pool)
