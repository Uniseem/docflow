...
pub async fn download_public(
    ...
    .await?;
    download_url(&state.network, url, destination, max_bytes, Some((state, id))).await?;
...
pub async fn extract_and_localize(
    ...
    for (index, reference) in refs.iter().enumerate() {
        let source = if reference.starts_with("http://") || reference.starts_with("https://") {
            ...
            download_url(&state.network, reference, &target, 50 * 1024 * 1024, None).await?;
            target
        } else {
            resolve_local_image(
                extract_root,
    ...
    for (reference, name) in &mapping {
        localized = localized.replace(reference, &format!("images/{name}"));
    }
    ...
    events::append(
        ...
        EventInput {
            ...
            detail: Some(&format!(
                "最终 {} 个唯一 WebP；文章图片全部使用相对路径，不依赖 MinerU 图片链接",
                names.len()
            )),
