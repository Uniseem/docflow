...
export function createIpcHandlers(ctx: HandlerContext): IpcHandlers {
  return {
    ...
    'app:takePendingFiles': () => handleAppTakePendingFiles(ctx),
    ...
    'glossaries:import': () => handleGlossariesImport(ctx),
    'glossaries:update': (req) => handleGlossariesUpdate(ctx, req),
    'glossaries:delete': (req) => handleGlossariesDelete(ctx, req),
    ...
    'shell:revealExport': (req) => handleShellRevealExport(ctx, req),
