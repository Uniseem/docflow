...
4.1 把 BabelDOC（PDFMathTranslate-next 2.9.0 的引擎）的其余优点也搬了过来：多段合并请求并带标题上下文，自动提取术语表统一译名（也能导入自己的术语表），粗体、斜体、彩色链接等段内格式保留，译文字体按原文粗细与衬线挑选，双语对照左右并排，可以只翻译指定页，带 OCR 文字层的扫描件可以白底黑字处理。
...
2. **翻译**：把 PDF 拖进窗口（或点「新建翻译」），选择模型后开始；只要其中几页时填「页码范围」（如 `1-3,5,8-`）。可以一次拖入多份，同时处理的文档数在「设置 → 通用」里调整。
3. **查看与导出**：处理完成后在右侧预览「中文」「双语对照」「原文」，用「导出」保存 PDF、自动提取的术语表（CSV）或打包 ZIP。处理记录里能看到每个阶段的进展、用量和警告。
4. **术语与排版**：「设置 → 高级」里可以导入自己的术语表（CSV，source、target 两列）、填写角色提示词（如「你是化学领域的专业译者」），选择双语对照的排法与译文字体。
5. **失败与重试**：网络中断、限流会自动重试；Key 无效、模型不存在、加密 PDF、扫描件等会直接给出原因和下一步。失败或已取消的文档可以「重新处理」，已翻译的段落不会重复计费。关闭应用后再打开，进行中的任务从断点继续。
...
3. 分隔竖线、图标等图形留在原位，译文重排后可能与它们交叠。
4. 译文比原文长很多时整篇字号会缩小；极少数任何字号都放不下的段落不写入（处理记录里有警告）。
5. 表格里的文字不翻译（BabelDOC 0.6 也已停用表格翻译）。
6. 没做过 OCR 的扫描件（只有图片、没有文字层）不支持，DocFlow 不做 OCR；带 OCR 文字层的扫描件要在「设置 → 高级」打开「自动处理带文字层的扫描件」，译文白底黑字，不保留段内格式。
7. 段内格式靠模型在译文里保留 `<style>` 标记；模型漏掉或弄乱标记时，这部分文字按段落的基本样式排（多出来的标记会被去掉）。
...
```bash
npm ci
npm run assets   # 下载版面模型与字体（约 170 MB，dev/build 前也会自动检查）
npm run dev      # 开发模式
