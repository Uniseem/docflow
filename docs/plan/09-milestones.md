...
## M10 照搬后继版本的其余优点（4.1.0，维护者要求）

pdf2zh 的后继 PDFMathTranslate-next 2.9.0 / BabelDOC 0.6.4 里 ADR-0017 没搬的优点照搬过来（[ADR-0018](../adr/0018-port-babeldoc-features.md)），之后发布 4.1.0。

- [ ] M10-1 分析 v5：段落组成（文字字符、样式序号、虚空格、公式序号）、基准样式、版面类别；原字体粗体/斜体/等宽/衬线（MuPDF.js）；单测。
- [ ] M10-2 翻译：BabelDOC 占位符、多段 JSON 请求（跨页、跨栏、200 token / 6 段）、上下文、过滤、译文检查与逐段回退、o200k token 计数、缓存键；设置迁移；单测（mock 服务）。
- [ ] M10-3 术语表：自动术语抽取、用户术语表（CSV 导入、启用、删除）、提示词里的术语表、`output/glossary.csv`；设置界面；单测。
- [ ] M10-4 写回：字体资源下载脚本、FontMapper、样式单元、原字形单元与整段原样、译文字体选项、OCR 白底；单测与维护者论文目检。
- [ ] M10-5 输出：双语并排/交替页、译文在前、书签迁移、页码范围、只输出所选页；新建翻译与设置界面；单测。
- [ ] M10-6 扫描件：DetectScannedFile（SSIM）与自动 OCR workaround；inspect 调整；单测（带 OCR 文字层的 fixture）。
