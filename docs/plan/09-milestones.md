...
- [x] M10-1 分析 v5：段落组成（文字字符、样式序号、虚空格、公式序号）、基准样式、版面类别；原字体粗体/斜体/等宽/衬线（MuPDF.js）；单测。
- [x] M10-2 翻译：BabelDOC 占位符、多段 JSON 请求（跨页、跨栏、200 token / 6 段）、上下文、过滤、译文检查与逐段回退、o200k token 计数、缓存键；设置迁移；单测（mock 服务）。
- [x] M10-3 术语表：自动术语抽取、用户术语表（CSV 导入、启用、删除）、提示词里的术语表、`output/glossary.csv`；设置界面；单测。
