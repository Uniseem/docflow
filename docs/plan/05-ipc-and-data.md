...
| compose   | warning：`第 P 页有公式字符找不到原字体，未能重画`（`font_unmapped`），未知 code → `第 P 页生成时出现警告`（没有页号时 `生成 PDF 时出现警告`；detail：`<code>: <warning 自带的 message>`）；info：`改写内容流：删除原文文字指令 M 条，写入 N 段译文，原样重绘公式 K 处`                 |
