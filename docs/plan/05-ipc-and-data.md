...
| compose   | warning：`第 P 页第 Q 段译文超出原段落范围`（`overflow`）、`第 P 页第 Q 段排版失败，已保留原文`（`layout_failed`）、`第 P 页第 Q 段的公式字体无法映射，已保留原文`（`font_unmapped`）、`第 P 页第 Q 段译文无法用内置字体写入，该页未写入译文`（`encode_failed`）、`第 P 页有文字指令无法对应到段落，已跳过该页`（`page_skipped`）、`第 P 页删除的原文指令数与预期不一致，请检查该页译文是否与原文重叠`（`op_mismatch`）、`中文字体子集嵌入失败，已改为嵌入完整字体，文件会大一些`（`font_subset_fallback`），未知 code → `第 P 页生成时出现警告`（没有页号时 `生成 PDF 时出现警告`；detail：`<code>: <warning 自带的 message>`）；Q 是该段在本页段落中的序号（从 1 起，按阅读顺序），找不到时写成 `第 P 页有一段…`；info：`改写内容流：写入 N 段译文，重绘公式 K 处，删除文字指令 M 条` |
...
| `shell:revealExport`     | `{ path }`                                                                                                                                         | `{}`（在访达/资源管理器中显示导出的文件。只接受本次运行中 `documents:export` 写出过的路径（含 `bundle` 的 ZIP），其他路径 → `not_found`：`找不到这个导出的文件。`；文件已被移动或删除 → `not_found`：`导出的文件已被移动或删除。`）（2026-09-23 M5 复查第 15 条：导出提示原先打开的是文档库副本）                                                                                                                                                                                                                                                                                                                                                                                         |
| `shell:openLogs`         | —                                                                                                                                                  | `{}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
文档状态变为 `completed`/`failed` 且主窗口不在前台（`isFocused()` 为 false）时，`new Notification({ title: '翻译完成' | '处理失败', body: title })`，点击后显示并前置主窗口（窗口已关闭时重新打开；不会选中该文档）。`settings.notifications=false` 或系统不支持通知时不发；没有窗口（macOS 关闭窗口后应用仍在运行）按不在前台处理，照样发。macOS Dock 徽标 = 进行中（queued/processing/retrying）的文档数（`app.dock.setBadge`，没有时清除）；进度条用 `win.setProgressBar(这些文档 progress 的平均值 / 100)`（Windows 任务栏，macOS 显示在 Dock 图标上），没有进行中的文档时设为 -1 清除。两者随 `document:changed` 的节流推送一起刷新。
